...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  6 12:08:27 2017
...
search_handle = Entrez.esearch(db="nucleotide", term='"insulin"[PROTEINFULLNAME] AND "arthropods"[porgn] AND 0:30000[Sequence Length]',
...
print(IDs)
out_handle = open("insulin-arth.txt", "w")       # open file where you save the result to                                 !<<----Check!
for seq_id in IDs:
...
tree = ET.parse("Blastres-insulin-trX3.xml")                                        #!<<----Check!
root = tree.getroot()
query = root[8]     #assing new object 'query' to root[8] for convenience
res = open("Blastres-insulin-selected.txt", "w")                                #!<<----Check!
