...
# -*- coding: utf-8 -*-
"""
Created on Sun Sep 24 00:33:27 2017
...
search_handle = Entrez.esearch(db="nucleotide", 
                               term='"glutathione S transferase"[TITL] AND "gst"[GENE] AND "arthropods"[porgn] AND 100:3000[Sequence Length] AND biomol_mrna[PROP]', 
                               usehistory="y", idtype="acc",RetMax=4000) # open search and search taeget genes in arthropod (limit sequence length less than 10kbp)   !<<----Check!
...
out_handle = open("GST-arth.txt", "w")       # open file where you save the result to                                 !<<----Check!
...
tree = ET.parse("Blastres-GST-trX3.xml")                                        #!<<----Check!
root = tree.getroot()
query = root[8]     #assing new object 'query' to root[8] for convenience
res = open("Blastres-GST-selected.txt", "w")                                #!<<----Check!
...
##############
#
# There seems to be no cDNA showing homology to Gluathione S-transferase
#
#
#
