...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 10 16:13:51 2018
...
search_handle = Entrez.esearch(db="nucleotide", term='"yolk protein"[PROTEINFULLNAME] AND "arthropods"[porgn] AND 0:30000[Sequence Length]',
...
print(IDs)
out_handle = open("yolkprotein-arth.txt", "w")       # open file where you save the result to                                 !<<----Check!
for seq_id in IDs:
...
# blastn -db Gryllus_rubens_bimac_firmus_TranscriptomeX3 -query yolkprotein-arth.txt -out Blastres-yolkprotein-TrX3.xml -outfmt 5 -evalue 0.5



# %% <<<<<  Filter BLAST result, saving to a text file >>>>>>>>
import xml.etree.ElementTree as ET
tree = ET.parse("Blastres-yolkprotein-trX3.xml")                                        #!<<----Check!
root = tree.getroot()
query = root[8]     #assing new object 'query' to root[8] for convenience
res = open("Blastres-yolkprotein-selected.txt", "w")                                #!<<----Check!
