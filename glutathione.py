...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 20 18:15:29 2018
...
search_handle = Entrez.esearch(db="nucleotide", term='"glutathione"[PROTEINFULLNAME] AND "arthropods"[porgn] AND 0:10000[Sequence Length]',
...
#print(IDs)
out_handle = open("glutathione-arth.txt", "w")       # open file where you save the result to                                 !<<----Check!
for seq_id in IDs:
...
tree = ET.parse("Blastres-glutathione-trX3.xml")                                        #!<<----Check!
root = tree.getroot()
query = root[8]     #assing new object 'query' to root[8] for convenience
res = open("Blastres-glutathione-selected.txt", "w")                                #!<<----Check!
