...
# -*- coding: utf-8 -*-
"""
Created on Sat Sep 23 15:35:16 2017
...
search_handle = Entrez.esearch(db="nucleotide", term='"glutathione peroxidase"[All field] AND "arthropods"[porgn] AND 0:30000[Sequence Length]',
...
print(IDs)
out_handle = open("GPx-arth.txt", "w")       # open file where you save the result to                                 !<<----Check!
for seq_id in IDs:
...
tree = ET.parse("Blastres-GPx-trX3.xml")                                        #!<<----Check!
root = tree.getroot()
query = root[8]     #assing new object 'query' to root[8] for convenience
res = open("Blastres-GPx-selected.txt", "w")                                #!<<----Check!
