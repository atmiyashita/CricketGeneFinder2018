...
search_handle = Entrez.esearch(db="nucleotide", term='"catalase"[PROTEINFULLNAME] AND "arthropods"[porgn] AND 0:30000[Sequence Length]',
...
print(IDs)
out_handle = open("Catalase-arth.txt", "w")       # open file where you save the result to                                 !<<----Check!
for seq_id in IDs:
...
tree = ET.parse("Blastres-Catalase-trX3.xml")                                        #!<<----Check!
root = tree.getroot()
query = root[8]     #assing new object 'query' to root[8] for convenience
res = open("Blastres-Catalase-selected.txt", "w")                                #!<<----Check!
