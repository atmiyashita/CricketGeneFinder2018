...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 22 13:57:58 2017
...
search_handle = Entrez.esearch(db="nucleotide", term='"glutathione"[PROTEINFULLNAME] AND "transferase"[PROTEINFULLNAME] AND "arthropods"[porgn] AND 200:2000[Sequence Length]',
...
#print(IDs)
out_handle = open("glutathioneANDtransferase-arth.txt", "w")       # open file where you save the result to                                 !<<----Check!
for seq_id in IDs:
...
tree = ET.parse("Blastres-glutathioneANDtransferase-trX3.xml")                                        #!<<----Check!
root = tree.getroot()
query = root[8]     #assing new object 'query' to root[8] for convenience
res = open("Blastres-glutathioneANDtransferase-selected.txt", "w")                                #!<<----Check!
