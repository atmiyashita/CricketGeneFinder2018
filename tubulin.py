...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 16 11:34:38 2017
...
search_handle = Entrez.esearch(db="nucleotide", term='"tubulin"[PROTEINFULLNAME] AND "arthropods"[porgn] AND 0:5000[Sequence Length]',
...
#print(IDs)
out_handle = open("tubulin-arth.txt", "w")       # open file where you save the result to                                 !<<----Check!
for seq_id in IDs:
...
tree = ET.parse("Blastres-tubulin-trX3.xml")                                        #!<<----Check!
root = tree.getroot()
query = root[8]     #assing new object 'query' to root[8] for convenience
res = open("Blastres-tubulin-selected.txt", "w")                                #!<<----Check!
