...
search_handle = Entrez.esearch(db="nucleotide", term='"insulin-like peptide"[PROTEINFULLNAME] AND "arthropods"[porgn] AND 0:500[Sequence Length]',
...
print(IDs)
out_handle = open("ilp-arth.txt", "w")       # open file where you save the result to                                 !<<----Check!
for seq_id in IDs:
...
tree = ET.parse("Blastres-ilp-trX3.xml")                                        #!<<----Check!
root = tree.getroot()
query = root[8]     #assing new object 'query' to root[8] for convenience
res = open("Blastres-ilp-selected.txt", "w")                                #!<<----Check!
