...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  6 12:16:47 2017
...
search_handle = Entrez.esearch(db="nucleotide", term='"insulin-like peptide"[PROTEINFULLNAME] AND "arthropods"[porgn] AND 501:50000[Sequence Length]',
...
print(IDs)
out_handle = open("ilp-large-arth.txt", "w")       # open file where you save the result to                                 !<<----Check!
for seq_id in IDs:
...
tree = ET.parse("Blastres-ilp-large-trX3.xml")                                        #!<<----Check!
root = tree.getroot()
query = root[8]     #assing new object 'query' to root[8] for convenience
res = open("Blastres-ilp-large-selected.txt", "w")                                #!<<----Check!
