...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 11 17:07:48 2017
...
search_handle = Entrez.esearch(db="nucleotide", 
                               term='"thioredoxin"[TITL] AND "arthropods"[porgn] AND 100:3000[Sequence Length] AND biomol_mrna[PROP]', 
...
out_handle = open("txn-arth.txt", "w")       # open file where you save the result to                                 !<<----Check!
...
tree = ET.parse("Blastres-TXN-trX3.xml")                                        #!<<----Check!
root = tree.getroot()
query = root[8]     #assing new object 'query' to root[8] for convenience
res = open("Blastres-TXN-selected.txt", "w")                                #!<<----Check!
